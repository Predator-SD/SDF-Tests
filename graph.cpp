...
Data Phi(double x,double y){
	Data r1=(Data){CSDF(x,y,-0.2,-0.2,0.1),18.0,0.0,0.0};
	//Data r2=(Data){RSDF(x,y,0.5,0.5,0.3,0.2,PI/12.0),0.01,0.3,1.5};
...
DWORD WINAPI solve1(LPVOID pm){
	int y1=0,y2=H/4;
	unsigned char *p=img+3*W*y1;
	double ans;
    for(int y(y1);y<y2;++y,pixels+=W)
        for(int x(0);x<W;++x,p+=3)
            ans=Integrate((double)x/W,(double)y/H),
            p[0]=(int)(minn(ans*102.0,102.0)),
            p[1]=(int)(minn(ans*204.0,204.0)),
            p[2]=(int)(minn(ans*255.0,255.0));
    return 0;
}
...
int main() {
    ...
    while(pixels<W*H) _sleep(5000),printf("Now: %lf\%\n",100.0*pixels/(W*H));
    svpng(fopen("result.png","wb"),W,H,img,0);
}
